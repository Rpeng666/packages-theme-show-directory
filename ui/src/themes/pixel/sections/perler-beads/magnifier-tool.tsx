...
export interface PerlerMagnifierToolProps {
  isActive: boolean;
  onToggle: () => void;
...
const MagnifierTool: React.FC<PerlerMagnifierToolProps> = ({
  isActive,
  onToggle,
  mappedPixelData,
  selectedColor,
  ...
  return (
    <>
      {/* 选择区域提示 */}
      {!selectionArea && (
        <div className="fixed top-4 left-1/2 transform -translate-x-1/2 bg-blue-500 text-white px-4 py-2 pxl-corner-sm shadow-lg z-[70]">
      ...
      {selectionArea && (
        <div
          ref={magnifierRef}
          className={`fixed bg-retro-surface/30 border-2 border-foreground/15 pxl-corner-md shadow-2xl border border-gray-200 dark:border-gray-600 select-none ${
        ...
        >
          ...
          {/* 放大视图内容 */}
          <div className="p-3">
            <div className="border border-foreground/20 pxl-corner-sm overflow-auto max-h-96">
              <canvas
                ref={canvasRef}
            ...
            {/* 当前选中颜色信息 */}
            {selectedColor && (
              <div className="mt-2 p-2 bg-gray-100 dark:bg-gray-700 pxl-corner-sm">
