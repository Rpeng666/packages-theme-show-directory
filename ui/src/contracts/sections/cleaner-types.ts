/**
 * Cleaner section display types — shared between the default and pixel themes.
 * Hoisted out of the pixel cluster (themes/pixel/sections/cleaner/types.ts) so
 * both theme implementations read the same structural shapes and the app can
 * type `analyzeResult` without double-sourcing.
 *
 * Kept minimal (only what the UI reads); the app's richer domain types
 * (AnalysisResult, etc.) stay app-side and are passed in as already-shaped
 * data.
 */
...
/** Issue row shape the analyze panel renders. */
